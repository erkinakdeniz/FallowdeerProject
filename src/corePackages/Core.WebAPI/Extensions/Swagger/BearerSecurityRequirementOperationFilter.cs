...
public class BearerSecurityRequirementOperationFilter : IOperationFilter
{
    public void Apply(OpenApiOperation operation, OperationFilterContext context)
    {
        ...
        if (!context.MethodInfo.GetParameters().Any(x => x.ParameterType.GetInterfaces().Contains(typeof(IRoleRequest)) || x.ParameterType.GetInterfaces().Contains(typeof(ISecured))))
            return;
