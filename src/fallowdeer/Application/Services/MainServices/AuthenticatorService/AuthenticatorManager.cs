...
public class AuthenticatorManager : IAuthenticatorService
{
    ...
    public async Task<EmailAuthenticator> CreateEmailAuthenticator(User user)
    {
        ...
        EmailAuthenticator emailAuthenticator =
           ...
           {
               UserId = user.Id,
               ActivationKey = RandomGenerator.CreateRandomCode(),
               IsVerified = false
           };
    ...
    public async Task SendAuthenticatorCode(User user, AuthenticatorType authenticatorType)
    {
        if (authenticatorType == AuthenticatorType.Email)
            await SendAuthenticatorCodeWithEmail(user);
    }

    public async Task VerifyAuthenticatorCode(User user, string authenticatorCode, AuthenticatorType authenticatorType)
    {
        if (authenticatorType == AuthenticatorType.Email)
            await VerifyAuthenticatorCodeWithEmail(user, authenticatorCode);
        else if (authenticatorType == AuthenticatorType.Otp)
    ...
    private async Task SendAuthenticatorCodeWithEmail(User user)
    {
        ...
        _mailService.SendMail(
            ...
            {
                ToList = toEmailList,
                Subject = "Authenticator Code - Kodkop Teknoloji",
